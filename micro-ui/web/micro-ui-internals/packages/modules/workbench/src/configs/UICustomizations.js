...
export const UICustomizations = {
  ...
  SearchIndividualConfig: {
    ...
    //         //const formattedAddress = [doorNo, street, locality, city, pincode].filter(Boolean).join(', ');
    ...
    //         // Extract locality name from each object in the array
    //         const localityNames = locality.map(loc => loc.name).filter(Boolean);
    
    //         const formattedAddress = [
    //           String(doorNo),
    //           String(street),
    //           localityNames.length > 0 ? localityNames.join(', ') : null,
    //           String(city),
    //           String(pincode)
    //         ].filter(Boolean).join(', ');
